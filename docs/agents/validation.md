...
2. Run `pwsh scripts/check-agents-md.ps1` to gate the instruction architecture (dead routes + banned phrases hard-fail; size is advisory).
3. Run `rg --files` to confirm expected docs exist.
4. Run `rg -n "docs/agents|AGENTS.md|repo-map.md|commands.md|boundaries.md|validation.md|security.md" AGENTS.md README.md docs/agents` to confirm routing references.
5. Run `git diff --check` to catch whitespace errors in tracked diffs.
6. Run `rg -n "[ \t]+$" README.md AGENTS.md CLAUDE.md docs/agents` to catch trailing whitespace in tracked and untracked Markdown files.
7. Read the final diff for public-safety and provenance issues.
...
| `check-agents-md.ps1` | Confirms every routed `.md` path resolves and no banned generic phrase crept into root; flags oversize root as advisory. |
